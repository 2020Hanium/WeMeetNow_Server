...
socket.on('add_friend', function(data) {
    ...
    socket.on('search_friend', function() { 
        ...
        socket.on('join_party', function(join_data) {
            ...
            var joinparty_sql = 'INSERT INTO party_member (party_member, party_head, party_name) VALUES (?, ?, ?)';
            var param = [member, head, party_name];

            connection.query(joinparty_sql, param, function(err, result) {
                if (err) {
                    console.log(err);
                    ...
                    socket.join(party_name);
                    socket.broadcast.to(party_name).emit('come_in');
                }
            })
